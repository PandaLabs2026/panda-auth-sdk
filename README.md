...
[项目配置](src/PandaAuth.Sdk/PandaAuth.Sdk.csproj)现支持生成 `0.2.0-preview.1` 预发行包，包内包含本 README、根目录 MIT 许可证正文和许可证元数据；预发行包尚未发布到 NuGet，也不构成 1.0 支持承诺。测试位于 [PandaAuth.Sdk.Tests](tests/PandaAuth.Sdk.Tests)。
